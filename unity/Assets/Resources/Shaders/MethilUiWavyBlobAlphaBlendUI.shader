Shader "UI/MethilUiWavyBlob_DoubleFill"
{
    Properties
    {
        _MainTex ("Texture", 2D) = "white" {}
        _FillColor ("Fill Color", Color) = (1,1,1,1)
        _InnerFillColor ("Inner Fill Color", Color) = (1,0,0,1)
        _BorderColor ("Border Color", Color) = (0,1,0,1)

        [Header(Fill Control)]
        _FillAmount ("Vertical Fill Amount", Range(0,1)) = 1.0
        _InnerFillAmount ("Horizontal Inner Fill", Range(0,1)) = 1.0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings IN) : SV_Target
            {
                ...
                // Vertical fill mask
                float fillMask = smoothstep(0.0, 0.001, uv.y - (1.0 - _FillAmount));
                float mainShapeVert = mainShape * fillMask;

                // Horizontal inner fill mask inside vertical fill (Left to Right)
                float innerFillMask = smoothstep(0.0, 0.001, _InnerFillAmount - uv.x); // left-to-right
                float mainShapeInner = mainShapeVert * innerFillMask;

                // Sample texture
                float4 texColor = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, uv);

                // Colors
                float4 outerFill = texColor * _FillColor;
                float4 innerFill = texColor * _InnerFillColor;

                // Combine: inner color overrides outer inside inner fill area
                float4 col = lerp(outerFill, innerFill, mainShapeInner);

                // Add border
                col = lerp(_BorderColor * shadowRing, col, mainShapeVert);
                col.a = saturate(shadowRing * _BorderColor.a + mainShapeVert * col.a);
